...
namespace Gosu
{
  ...
  public:
    MPEGFile(Gosu::Reader reader)
    {
      ...
      if (mpg123_scan(handle_) == MPG123_ERR)
        throw std::runtime_error("Cannot estimate mpg123 file's length: ");
    ...
    std::size_t read_data(void* dest, std::size_t length) override
    {
      ...
      while (written == 0
        ...
        mpg123_feed(handle_, static_cast<unsigned char*>(contents_.data()) + position_, bytes);
        position_ += bytes;
        error = mpg123_read(handle_, static_cast<unsigned char*>(dest), length, &written);
      }
      return written;
