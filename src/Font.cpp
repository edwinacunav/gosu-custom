...
Gosu::Font::Font(int font_height, const string& font_name, unsigned font_flags)
{
  ...
  pimpl->text = "Temporary Placeholder";
  pimpl->color = new Gosu::Color(255, 255, 255, 255);
}

Gosu::Font::~Font() { delete pimpl; }
...
const string& Gosu::Font::text() const
{
  return pimpl->text;
}

Gosu::Color* Gosu::Font::color() const
{
  return pimpl->color;
}

bool Gosu::Font::visible() const
{
  return pimpl->visible;
}

void Gosu::Font::set_text(const string& text)
{
  pimpl->text = text;
}

void Gosu::Font::set_color(Gosu::Color* color)
{
  pimpl->color = color;
}

void Gosu::Font::set_visible(bool seen)
{
  pimpl->visible = seen;
}
...
double Gosu::Font::set_text_width() const
{
  double width = 0;
  MarkupParser parser(pimpl->base_flags, false, [&](vector<FormattedString>&& line) {
    double line_width = 0;
  ...
  MarkupParser parser(pimpl->base_flags, false, [&](vector<FormattedString>&& line) {
    ...
    for (auto& part : line) {
      for (auto codepoint : part.text) {
        auto& image = pimpl->image(codepoint, part.flags);
        double image_scale = 1.0 * height() / image.height();
        image.draw(current_x, current_y, z, image_scale * scale_x,
                   image_scale * scale_y, multiply(c, part.color), mode);
